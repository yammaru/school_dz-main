(function ($) {
    ...
    $('body').on('click', '.js-add-study_plan_chapter', function (e) {
        const $this = $(this);
        ...
        var clone = $('#studyPlanChapterModalHtml').clone();
        clone.removeClass('d-none');
        var cloneHtml = clone.prop('innerHTML');
        cloneHtml = cloneHtml.replaceAll('record', random);

        clone.html('<div id="studyPlanChapterModal' + random + '">' + cloneHtml + '</div>');
        ...
        Swal.fire({
            ...
            onOpen: function () {
                ...
                const modal = $('#studyPlanChapterModal' + random);

                modal.find('input.js-study-chapter-webinar-id').val(webinarId);

                if (itemId) {
                    modal.find('.section-title').text(editChapterLang);
                    modal.find('.js-ajax-title').val(title);
                    modal.find('.js-ajax-content').val(content);
                    const path = adminPanelPrefix + '/study_plan_chapters/' + itemId + '/update';
                    modal.find('.study-chapter-form').attr('data-action', path);
                }
            }
        });
    });
    ...
    $('body').on('click', '.js-add-course-content-btn, .add-new-interactive-file-btn' , function (e) {
        ...
        switch (type) {
            ...
            case 'study_plan':
                const newStudyPlanTextLessonForm = $('#newStudyPlanTextLessonForm');
                newStudyPlanTextLessonForm.find('.chapter-input').val(chapterId);
                html = newStudyPlanTextLessonForm.html();
    ...
    /////////////////////
    $('body').on('click', '.js-add-study-plan-content-btn' , function (e) {
        e.preventDefault();
        const $this = $(this);
        const type = $this.attr('data-type');
        const chapterId = $this.attr('data-chapter');

        const contentTagId = '#studyChapterContentAccordion' + chapterId;
        const key = randomString();
        var html = '';
    ...
    ////
    $('body').on('click', '.js-change-study-content-chapter', function (e) {
        e.preventDefault();
        const $this = $(this);
        const itemId = $this.attr('data-item-id');
       /* const itemType = $this.attr('data-item-type');*/
        ...
        var clone = $('#changeStudyPlanChapterModalHtml').clone();
        clone.removeClass('d-none');
        var cloneHtml = clone.prop('innerHTML');

        clone.html('<div id="changeStudyPlanChapterModalHtml' + random + '">' + cloneHtml + '</div>');
        ...
        Swal.fire({
            ...
            onOpen: function () {
                ...
                const modal = $('#changeStudyPlanChapterModalHtml' + random);

                modal.find('input.js-item-id').val(itemId);
                /*modal.find('input.js-item-type').val(itemType);*/
                modal.find('.js-ajax-study_chapter_id').val(chapterId).change();
            }
        });
    });

    $('body').on('click', '.save-change-study-plan-chapter', function (e) {
        e.preventDefault();
        const $this = $(this);
        let form = $this.closest('.study-change-chapter-form');

        handleStudyPlanChapterForm(form, $this);
    });
    ////
   ...
   * */
    ...
    * */
    ...
    window.handleStudyPlanChapterForm = function (form, $this) {
        let data = serializeObjectByTag(form);
        let action = form.attr('data-action');
    ...
    $('body').on('click', '.save-study-plan-chapter', function (e) {
        e.preventDefault();
        const $this = $(this);
        let form = $this.closest('.study-chapter-form');
