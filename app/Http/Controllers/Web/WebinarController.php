...
class WebinarController extends Controller
{
    ...
    public function course($slug, $justReturnData = false)
    {
        ...
        $webinarStudyPlanCount = 0;
        if (!empty($course->studyPlanChapters)) {
            $webinarStudyPlanCount += $course->studyPlanChapters->count();
        }
        ...
        $advertisingBanners = AdvertisingBanner::where('published', true)
            ...
            ->get();
        if (!empty($user)) {
            $attendances= AttendanceTranslation::where('student_id',$user->id)
                ->join('attendances', 'attendances.id', '=', 'attendance_translations.attendance_id')
                ->select( 'attendance_translations.*','attendances.date','attendances.updated_at','attendances.text_lesson_id')
                ->get();
        }
        ...
        $data = [
            ...
            'webinarStudyPlanCount'=>$webinarStudyPlanCount,
