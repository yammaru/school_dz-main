...
class WebinarController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $this->validate($request, [
            'type' => 'required|in:webinar,course,text_lesson,classroom_course',
    ...
    public function edit(Request $request, $id)
    {
        ...
        $data = [
            ...
            'studyPlanChapters' => $webinar->studyPlanChapters,
        ...
        if (!empty($itemIds) and is_array($itemIds) and count($itemIds)) {
            switch ($tableName) {
                ...
                case 'webinar_chapters':
                    ...
                    break;
                case 'study_plan_chapters':
                    foreach ($itemIds as $order => $id) {
                        StudyPlanChapter::where('id', $id)
                            ->update(['order' => ($order + 1)]);
                    }
                    break;
                case 'study_plan_text_lessons':
                    foreach ($itemIds as $order => $id) {
